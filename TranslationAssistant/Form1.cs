...
namespace TranslationAssistant
{
	public partial class Form1 : Form
	{
		...
		private void dictionaryToolStripMenuItem_Click(object sender, EventArgs e)
		{
			...
			try
			{
				webBrowser.Navigate("http://jpdic.naver.com/search.nhn?range=all&q=" + Uri.EscapeDataString(richTextBox.SelectedText) + "&sm=jpd_hty");
			}
			catch
			{
			}
		...
		private void dictionaryLineToolStripMenuItem_Click(object sender, EventArgs e)
		{
			...
			try
			{
				webBrowser.Navigate("http://jpdic.naver.com/search.nhn?range=all&q=" + Uri.EscapeDataString(richTextBox.Lines[clickedLineNumber - 1]) + "&sm=jpd_hty");
			}
			catch
			{
			}
		...
		private void webBrowser_DocumentCompleted(object sender, WebBrowserDocumentCompletedEventArgs e)
		{
			...
			if (webBrowser.Url.ToString().StartsWith("http://jpdic.naver.com/search.nhn?range=all&q=") && webBrowser.Url.ToString().EndsWith("&sm=jpd_hty"))
			{
		...
		private void TranslateLine()
		{
			...
			try
			{
				webBrowser.Navigate("http://jpdic.naver.com/search.nhn?range=all&q=" + Uri.EscapeDataString(richTextBox.Lines[lineNumber - 1]) + "&sm=jpd_hty");
			}
			catch
			{
			}
